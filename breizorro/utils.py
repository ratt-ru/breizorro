...
def parse_fov(fov_str):
    ...
    try:
        ...
        if len(parts) != 4:
            raise ValueError(
                f"FOV must have exactly 4 comma-separated values [x0,y0,x1,y1], got {len(parts)} values"
            )
        ...
        if x0 < 0 or y0 < 0 or x1 <= x0 or y1 <= y0:
            raise ValueError(
                f"Invalid FOV range: x0={x0}, y0={y0}, x1={x1}, y1={y1}. "
...
def apply_fov_crop(mask, fov_str):
    ...
    if x0 >= x1 or y0 >= y1:
        raise ValueError(
            f"FOV range results in invalid crop after clamping: x0={x0}, x1={x1}, y0={y0}, y1={y1}"
        )
